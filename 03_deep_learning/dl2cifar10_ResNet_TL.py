...
def main():
    ...
    # 核心修改点：开启预训练外挂
    # weights="DEFAULT" 会自动下载这位“大学教授”的大脑记忆（约 40MB）
    model = models.resnet18(weights="DEFAULT")
    # 换成这三行：加载没经过预训练的空壳 ResNet18，并把最后的输出改成 10 类
    #model = models.resnet18(weights=None)
    
    # 教授原本习惯做 1000 道题，我们把他的输出切掉，接上我们的 10 分类考卷
    ...
    # ==========================================
    # 新增：将学霸的“大脑记忆”永久保存到硬盘上
    # ==========================================
    save_path = str(BASE_DIR / "assets" / "models" / ".data" / "resnet18_cifar10_best.pth")
    # 我们只保存 state_dict (也就是每个神经元的权重数字)，这是官方最推荐的做法
    torch.save(model.state_dict(), save_path)
    
    print(f"\n🎉 炼丹彻底竣工！模型权重已永久保存至: {save_path}")
    print("下次你可以直接加载这个 .pth 文件，不用再重新训练了！")
